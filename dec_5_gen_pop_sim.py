...
<body>
  <div class="container">
    ...
    <div class="warning">
      ...
      <p><strong>What we know with HIGHER CONFIDENCE (from CMC research):</strong><br>
    ...
    <div class="row">
      <div class="card settings-card">
        ...
        <div class="grid">
          ...
          <label>CMC Percentage</label>
          <div style="display:flex;align-items:center;gap:.5rem;">
            <input id="cmc_pct" type="number" value="0.5" min="0.01" max="1.0" step="0.01" style="width:60%;">
            <span style="font-size:.85rem;color:#7f8c8d;">%</span>
          </div>
        ...
        <div class="metric" style="background:#d4edda;">
          ...
          <span class="confidence-badge high-confidence">HIGHER CONFIDENCE</span>
        </div>
      </div>
...
<script>
  ...
  function simulate(){
      ...
      let cmcPct = parseFloat(document.getElementById('cmc_pct').value);
      
      // Validate and clamp CMC percentage to 0.01-1.0 range (AAP: <1% of children)
      if (isNaN(cmcPct) || cmcPct < 0.01) cmcPct = 0.01;
      if (cmcPct > 1.0) cmcPct = 1.0;
      document.getElementById('cmc_pct').value = cmcPct;  // Update display
      cmcPct = cmcPct / 100;  // Convert % to decimal
    ...
    // Calculate CMC population dynamically
    const cmcPopulation = Math.round(usPop * cmcPct);
    const nonCmcPop = usPop - cmcPopulation;  // Subtract CMC children
    const nonCmcSaved = scenario === 'none' ? 0 : Math.round((baselineMean - afterMean) * nonCmcPop);
    
    // Calculate CMC days saved (full strength effect)
    const cmcBeta = CMC_BETAS.loneliness;
    const cmcInterventionEffect = intervention.loneliness;
    const cmcDaysSaved = scenario === 'none' ? 0 : Math.round((cmcBeta * cmcInterventionEffect) * cmcPopulation);

    // Combined total: non-CMC (scaled) + CMC (full)
    const combinedTotal = nonCmcSaved + cmcDaysSaved;

    document.getElementById('days_saved').textContent = combinedTotal >= 0 ? combinedTotal.toLocaleString() : '0';

    // CMC breakdown display
    document.getElementById('cmc_comparison').textContent = 
        '~' + cmcDaysSaved.toLocaleString() + ' days (' + 
        cmcPopulation.toLocaleString() + ' CMC children)';

    // Calculate all scenarios for comparison
    const allScenarios = {};
    const cmcEffect = (CMC_BETAS.loneliness * (intervention.loneliness || 0));
    ...
    for(const [assumpName, assumpParams] of Object.entries(ASSUMPTIONS)){
      ...
      const s = beta * interv;
      const perChild = s;
      const nonCmcTotal = scenario === 'none' ? 0 : Math.round(perChild * nonCmcPop);
      const cmcTotal = scenario === 'none' ? 0 : Math.round(cmcEffect * cmcPopulation);
      const total = nonCmcTotal + cmcTotal;
      allScenarios[assumpName] = { perChild, total, cmcTotal, nonCmcTotal };
}

    document.getElementById('range_display').innerHTML = `
  <strong>Conservative:</strong> ${allScenarios.conservative.total.toLocaleString()} days<br>
  <span style="font-size:.85em;color:#666;margin-left:1rem;">CMC: ${allScenarios.conservative.cmcTotal.toLocaleString()} | Non-CMC: ${allScenarios.conservative.nonCmcTotal.toLocaleString()}</span><br>
  <strong>Middle:</strong> ${allScenarios.middle.total.toLocaleString()} days<br>
  <span style="font-size:.85em;color:#666;margin-left:1rem;">CMC: ${allScenarios.middle.cmcTotal.toLocaleString()} | Non-CMC: ${allScenarios.middle.nonCmcTotal.toLocaleString()}</span><br>
  <strong>Optimistic:</strong> ${allScenarios.optimistic.total.toLocaleString()} days<br>
  <span style="font-size:.85em;color:#666;margin-left:1rem;">CMC: ${allScenarios.optimistic.cmcTotal.toLocaleString()} | Non-CMC: ${allScenarios.optimistic.nonCmcTotal.toLocaleString()}</span>
`;
