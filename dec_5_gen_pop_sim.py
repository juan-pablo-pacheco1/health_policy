...
<script>
  ...
  function simulate(){
    ...
    // Separate CMC from general population to avoid double-counting
    const nonCmcPop = usPop - 1460000;  // Subtract CMC children
    const totalSaved = scenario === 'none' ? 0 : Math.round((baselineMean - afterMean) * nonCmcPop);
    
    // Show combined total: CMC (full effect) + non-CMC (scaled effect)
    ...
    for(const [assumpName, assumpParams] of Object.entries(ASSUMPTIONS)){
      ...
      const cmcTotal = scenario === 'none' ? 0 : Math.round(cmcEffect * cmcPop);
    ...
    <em style="font-size:.85em;color:#666;">(Includes CMC at full effect + non-CMC at scaled effect)</em>
  `;
